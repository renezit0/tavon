...
!macro DownloadAndInstall MODULE TAG FILENAME
  ...
  StrCpy $R0 "${BASE_URL}/${TAG}-v${PRODUCT_VERSION}/${FILENAME}-Setup-${PRODUCT_VERSION}.exe"
  StrCpy $R1 "$TEMP\${FILENAME}-Setup.exe"
  ExecWait `powershell.exe -NoProfile -Command "Invoke-WebRequest -Uri '$R0' -OutFile '$R1' -UseBasicParsing"` $0
  ${If} $0 == 0
